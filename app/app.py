...
def system_logs():
    # Only logged-in admins can see this
    if 'loggedin' not in session:
        return redirect(url_for('login'))
    ...
    start_date = request.args.get('start_date') or ''  # YYYY-MM-DD
    end_date = request.args.get('end_date') or ''      # YYYY-MM-DD

    # ---- get distinct sources + levels for dropdowns ----
    ...
    # ---- build main query with optional WHERE conditions ----
    query = """
        SELECT
            log_date,
            source,
